...
connector.execute(
'CREATE TABLE IF NOT EXISTS Users (ID INTEGER PRIMARY KEY NOT NULL, username TEXT, password TEXT, mobile TEXT, CARD_ID TEXT)'
)
...
Label(rt, text="Register Yourself", font="ar 15 bold", bg='SteelBlue', fg='White').grid(row=0, column=0, columnspan=6, sticky="ew")

id = Label(rt, text="ID", font="Georgia 10")
...
id.grid(row=1+1, column=1+1, sticky="w")
name.grid(row=3+1, column=1+1, sticky="w")
password.grid(row=5+1, column=1+1, sticky="w")
mobile.grid(row=7+1, column=1+1, sticky="w")
CardID.grid(row=9+1, column=1+1, sticky="w")

id_entry = Entry(rt)
name_entry = Entry(rt)
password_entry = Entry(rt, show="*")
mobile_entry = Entry(rt)
CardID_entry = Entry(rt)

id_entry.grid(row=1, column=3, sticky="ew")
name_entry.grid(row=3, column=3, sticky="ew")
password_entry.grid(row=5, column=3, sticky="ew")
mobile_entry.grid(row=7, column=3, sticky="ew")
CardID_entry.grid(row=9, column=3, sticky="ew")

# Label(rt, text="Register Yourself", font="ar 15 bold", bg='SteelBlue', fg='White').pack(side=TOP, fill=X)
...
# password.pack(side = TOP, expand = True, fill = BOTH)
# mobile.pack(side = TOP, expand = True, fill = BOTH)
# CardID.pack(side = TOP, expand = True, fill = BOTH)

# # Create Entry Widgets
# id_entry = Entry(rt)
# name_entry = Entry(rt)
# password_entry = Entry(rt, show="*")  # Use "show" to hide the password
# mobile_entry = Entry(rt)
# card_id_entry = Entry(rt)

# id_entry.pack(side=TOP, expand=True, fill=BOTH)
# name_entry.pack(side=TOP, expand=True, fill=BOTH)
# password_entry.pack(side=TOP, expand=True, fill=BOTH)
# mobile_entry.pack(side=TOP, expand=True, fill=BOTH)
# card_id_entry.pack(side=TOP, expand=True, fill=BOTH)
...
#     cursor.execute("INSERT INTO Users (username, password, mobile, CARD_ID) VALUES (?, ?, ?, ?)",
#                    (username, password, mobile, card_id))
#     connector.commit()
#     mb.showinfo("Registration", "User registered successfully!")

# register_button = Button(rt, text="Register", command=register_user)
# register_button.pack(side=TOP, expand=True, fill=BOTH)


# Label(rt, text="Register Yourself", font="ar 15 bold").grid(row=0,column=3)
# Label(rt, text="Register Yourself", font="ar 15 bold", bg='SteelBlue', fg='White').pack(side=TOP, fill=X)
# Functions
# def issuer_card():
